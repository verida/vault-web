...
const FolderPage = ({ params }: { params: { folder: string[] } }) => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (folder?.database) {
          const db = await webUserInstanceRef.current?.openDatabase(folder!.database);
  ...
  return (
    <div className='flex-col'>
      <Button
        variant={"ghost"}
        className='flex gap-5 text-sm font-medium items-center mb-4 h-16'
              ...
              <Avatar>
                {issuer?.avatar?.uri && (
                  <AvatarImage src={issuer?.avatar?.uri} asChild>
                    <Image src={issuer?.avatar?.uri} width={40} height={40} alt='Issuer avatar' />
                  </AvatarImage>
                )}
                <AvatarFallback />
              </Avatar>
